...
- 여러 배열을 각 배열의 같은 인덱스 요소들을 묶은 튜플의 배열로 만든다.
...
- `zip(...arrs)`
- 여러 배열의 같은 위치에 있는 요소들을 하나로 묶을 때 사용한다.
- 각 배열의 같은 인덱스에 있는 요소들을 튜플로 묶어 새 배열을 반환한다.
...
- 배열의 길이가 다르면 가장 긴 배열의 길이에 맞춘다. 짧은 배열의 빈자리는 `undefined`로 채운다.
...
- `arrs`(`Array<readonly T[]>`): 묶을 배열들이다.

**반환 값**

- (`T[][]`): 각 입랙 배열의 해당 인덱스 요소들을 튜플로 묶은 새 배열을 반환한다.
