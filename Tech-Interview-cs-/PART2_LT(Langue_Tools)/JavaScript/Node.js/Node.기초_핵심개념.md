...
### 3-3. Node.js와 브라우저 차이
- 브라우저와 Node.js는 모두 JavaScript를 사용하나 제공하는 환경은 다르다.
* **DOM 및 Web API 부재**: Node.js에 브라우저의 `docuiment`나 `window` 객체가 없다. 대신 파일 시스템, 네트워크, 프로세스 관련 API를 제공해 서버나 CLI 프로그램을 만들 수 있다.
* **모듈 시스템**: Node.js는 CommonJS(`require`)와 ES 모듈(`import`)을 모두 지원한다. 브라우저에는 주로 ES 모듈만 사용할 수 있다.
* **환경제어**: Node.js 애플리케이션은 실행 환경과 버전을 개발자가 직접 선택할 수 있다. 브라우저에 사용자의 브라우저 종류와 버전에 따라 지원하는 JavaScript 기능이 달라지고, Node.js에서 원하는 버전을 명시하여 최신 ES 표준을 사용할 수 있다.

### 3-4. Node.js 생태계와 NPM
- Node.js의 npm이 있다. npm은 Node.js 표준 패키지 매니저고, 꽤 많은 패키지가 등록되어 세계에서 큰 단일 언어 코드 저장소가 되었다.
- `npm install` 명령으로 프로젝트의 `package.json`에 명시된 모든 의존성을 쉽게 설치할 수 있고, 특정 패키지 또는 특정 버전을 설치/업데이트 하는 기능을 제공한다.
- 주요 프레임워크 라이브러리:
  * **Express.js**: 미들웨어 기반의 간결한 웹 프레임워크로 REST API 작성에 많이 사용된다.
  * **Fastify**: 높은 성능과 저비용 오버헤드를 목표로 하는 HTTP 프레임워크다.
  * **NestJS**: 모듈식 구조로 대규모 프로젝트에 적합하다.
  * **Socket.IO**: 실시간 양방향 통신을 위해 WebSocket을 추상화한 라이브러리다.
  * **Mongoose**: MongoDB ODM(Object Data Modeling) 라이브러리로 스키마 기반 데이터 모델링을 지원한다.

---

## 4. Node.js 버전, 릴리스 정책
- Node.js는 짝수 버전이 LTS(Long-Term Support) 버전으로 지정, 6개월간 Current(현재) 릴리스를 거친 후 Active LTS로 이동한다. 
- 실제 서비스에서는 Maintenance LTS 버전을 사용하는 것이 권장된다. 
- 현재 기준으로는 v24.12.0이 최신 버전이고 v25.2.1은 최신 Current 릴리스이다.
| 릴리스 상태 | 버전 예시 | 추천 용도 |
| :--- | :--- | :--- |
| **Active LTS** | v24.12.0 | 안정성이 중요한 서비스 운영에 적합 |
| **Current** | v25.2.1 | 최신 기능 테스트, 개인 학습용 |
| **Maintenance LTS** | v22.x | 향후 몇 년간 보안 업데이트만 제공 |

---

## 마무리
- Node.js는 JavaScript를 서버에서 실행하는 런타임 환경, V8 엔진과 libuv를 통해 높은 성능과 비동기 I/O를 제공한다.
- 한번 작성한 JavaScript 코드를 브라우저와 서버 양쪽에 공유할 수 있어 학습 곡선이 낮고 생산성이 높다.
- 단일 스레드 구조로 인해 CPU 계산이 많은 서버에는 적합하지 않고, 이벤트 루프를 막지 않도록 신경 써야 하는 단점이 있다.
- 풍부한 생태계와 npm 패키지, 다양한 프레임워크, 넓은 커뮤니티 덕분에 Node.js는 실시간 애플리케이션, API 서버, 마이크로서비스 등 웹 개발에 널리 사용된다.