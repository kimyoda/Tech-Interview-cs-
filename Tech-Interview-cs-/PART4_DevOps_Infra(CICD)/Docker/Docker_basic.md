...
- Docker는 애플리케이션을 일관된 환경에서 빠르게 배포하고 확장할 수 있게 해주는 핵심 도구이다.
- 컨테이너와 가상머신의 차이를 이해, 이미지 레이어와 데이터 저장 방식을 활용하면 효율적인 개발, 운영 환경을 구축할 수 있다.
- 브리지, 호스트, 오버레이, Macvlan, None 등 다양한 네트워크 드라이버를 상황에 맞게 선택해 컨테이너 간 통신을 최적화 해야한다.

- 최소 권한 이미지 사용: 이미지 크기를 줄이고 공격 표면을 줄이기 위해 필요하지 않은 패키지는 제거, `alpine`과 같은 경량 기반 이미지를 사용한다.
- 멀티 스테이지 빌드: 빌드 도구와 실행 환경을 분리해 최종 이미지 크기를 줄일 수 있다.
- 헬스 체크: `HEALTHCHECK` 명령을 사용해 컨테이너 상태를 모니터링, Compose 파일에서 의존성 서비스를 재시작하는 전략을 세운다.
