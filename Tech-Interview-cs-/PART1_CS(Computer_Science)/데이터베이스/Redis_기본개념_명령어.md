...
- `ZREVRANK key member`는 특정 멤버가 정렬된 집합에서 몇 동인지 0부터 시작하는 역순위를 반환한다.
- 존재하지 않는 멤버는 `nil`을 반호나하고, `WITHSCORE` 옵션을 통해 점수를 함께 얻을 수 있다.

**`ZCARD` - 집합의 크기 조회**

- `ZCARD key`는 정렬된 집합에 저장된 멤버 수(카디널리티)를 반환한다.
- 키가 없으면 0을 반환하므로 랭킹에 등록된 전체 사용자 수를 파악하는 데 사용한다.

**`HGETALL` - 해시의 모든 필드/값 조회**

- `HGETALL key`는 해시의 모든 필드와 값을 순서대로 반환한다.
- 반환 배열은 각 필드 다음에 값이 이어져 길이가 두 배가 된다.
- `HGETALL`을 통해 사용자 상태 해시의 모든 속성을 한 번에 확인할 수 있다.

**`TYPE` - 키의 데이터 타입 확인**

- `TYPE key`는 지정된 키에 저장된 값의 데이터 타입을 문자열 형태로 반환한다.
- 결과는 `string`, `list`, `set`, `zest`, `hash`, `stream`, `vectorset` 등의 값이고 키가 없으면 `none`이다.
- 예상과 다른 타입이 저장되어 있지 않은 지 확인할 때 유용하다.

**`SCAN` - 반복 가능한 키 검색**

- `SCAN cursor [MATCH pattern] [COUNT count]` 명령은 전체 데이터베이스를 블로킹하지 않고 키를 검색할 수 있게 한다.
- `MATCH` 옵션으로 접두사나 패턴을 지정해 관련 키만 찾을 수 있다.
- `COUNT` 옵션으로 한 번에 처리할 키의 수를 조절할 수 있어 운영 환경에서 안전하게 키를 탐색할 수 있다.

- 위 명령어들은 랭킹을 구현하거나 운영 중 데이터 상태를 점검할 때 필수적이다.
- `ZREVRANGE`와 `ZREVRANK`를 함께 사용하여 상위 플레이어 목록과 특정 플레이어 순위를 동시에 확인할 수 있고, `ZSCORE`로 점수 값을 검증, `ZCARD`로 전체 랭킹 규모를 파악한다.
- `HGETALL`과 `TYPE`은 특정 키의 데이터 구조와 내용을 확인하는 데 도움을 주고, `SCAN`을 이용하여 특정 접두사의 키를 안전하게 찾을 수 있다.

---

### 운영 쉘 및 Artisan 명령어

- Laravel 애플리케이션은 CLI 도구인 Artisan을 통해 데이터베이스 마그레이션, 캐시관리, 커스텀 명령 실행 등 다양한 운영 작업을 수행할 수 있다.
- 프로젝트 요구에 맞춘 명령도 정의할 수 있고, Redis 관련 캐시를 초기화하거나 복원하는 명령 기능을 활용한다.

---

### 커스텀 Artisan 명령(`app:restore-redis` 등)

- `php artisan app:restore-redis` 와 같이 `app:` 접두사로 시작하는 명령은 프로젝트에 정의한 커스텀 명령이다.
- `make:command`를 실행해 새로운 명령 클래스를 생성한다.
- 이 명령은 `app/Console/Commands` 디렉터리에 클래스를 생성하고, 해당 디렉터리가 없으면 자동으로 만들어 준다.
- 생성된 클래스의 `$signature`와 `$description` 속성을 정의하면 `php artisan list`에 명령 이름과 설명이 표시된다.
- `handle()` 메소드에 실제 로직을 구현한다.
